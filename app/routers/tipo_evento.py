...
from app.services.tipo_evento_service import TipoEventoService
...
def obtener_tipo_evento(tipo_evento_id: int, db: Session = Depends(get_db)):
    obj = TipoEventoService.obtener(db, tipo_evento_id)
    if not obj:
...
def actualizar_tipo_evento(
    ...
    return TipoEventoService.actualizar(db, obj, data)
...
def eliminar_tipo_evento(tipo_evento_id: int, db: Session = Depends(get_db)):
    ...
    return TipoEventoService.eliminar(db, obj)
