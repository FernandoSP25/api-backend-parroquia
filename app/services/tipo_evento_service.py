...
class TipoEventoService:
    ...
    def obtener(db: Session, tipo_evento_id: int):
        return db.query(TipoEvento).filter(TipoEvento.id == tipo_evento_id, TipoEvento.activo == True).first()

    @staticmethod
    def actualizar(db: Session, tipo_evento_id: int, data: TipoEventoUpdate):
    ...
    @staticmethod
    def eliminar(db: Session, tipo_evento_id: int):
        obj = TipoEventoService.obtener(db, tipo_evento_id)
        if obj:
            obj.activo = False
            db.commit()
